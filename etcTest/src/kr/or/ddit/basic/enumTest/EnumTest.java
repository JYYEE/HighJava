 ...
 *
 * 			// '변수명'의 개수는 '값들'의 개수와 같고, 자료형도 각각 맞아야 한다.
 * 				private 열거형이름(자료형 변수명,...){
 * 					위에 선언된 변수들을 초기화한다.
 * 					...
 * 				}
 *			// 구성된 '값들'을 외수에서 불러올 수 있는 getter메서드를 만든다.
 */


public class EnumTest {
	// 클래스 안에 내부 클래스처럼 만들기기
	public enum Color{RED, GREEN, BLUE} 
	public enum Count{ ONE, TWO, THREE}
	// ConstTest에서 만든 상수와는 연관x.
	public enum Season{
		// 상수명(값들,...) 형식의 선언
		봄("3월부터 5월까지", 15),
		여름("6월부터 8월까지", 32),
		...
		// 생성자
		Season(String span, int temp) { // ==> private이 있는 것과 같음.
			this.span = span;
			this.temp = temp;
		}
	...
	public static void main(String[] args) {
		...
		if(ConstTest.RED == ConstTest.THREE) { 
			// 문법적으로는 문제가 없지만 성격이 다른 것들을 비교한다는 것이 논리적으로는 맞지 않음.
			System.out.println("");
		} else {
			System.out.println("");
		}
...
//		}
		...
		// 상수명만 기술해야 한다.
		switch(mycnt) { // 케이스에 쓸 때는 열거문 이름 빼고 쓰기
