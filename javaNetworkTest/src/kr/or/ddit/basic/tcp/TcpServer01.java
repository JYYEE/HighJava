...
public class TcpServer01 {
	public static void main(String[] args) throws IOException {
		...
		// accept() 메서드
		//  ==> 클라이언트의 연결 요청이 올 때까지 계속 기다린다. (연결이 안오면 멈춰있는 것처럼 보임)
		...
		System.out.println("IP 주소 : " + socket.getInetAddress().getHostAddress()); // 소켓과 연결된 클라이언트의 IP 주소
		System.out.println("Port 번호 : " + socket.getPort()); // 포트는 시스템이 비어있는 곳에 자동 배정
		System.out.println();
		...
		// Socket 객체의 OutputStream 객체를 구해서 데이터를 전송한다.
		// (Socket객체의 getOutputStream() 메서드 이용)
		OutputStream out = socket.getOutputStream(); // 한글을 보내기 어려움
		DataOutputStream dout = new DataOutputStream(out);// 한글 보내는데 도움을 주는 보조 스트림
