...
// TCP 소켓 방식의 통신 프로그램으로 클라이언트 역할을 한다.
...
public class TcpClient01 {
	public static void main(String[] args) throws UnknownHostException, IOException {
		 ...
		 * 1. 원래의 IP주소 : 예) 192.168.146.51
		 * 2. 지정된 IP주소 : 127.0.0.1 (모든 컴퓨터에서 본인컴퓨터를 의미)
		 * 3. 원래의 컴퓨터 이름 : 예) DESKTOP-VROBB5F
		...
		// 서버와 클라이언트가 같은 컴퓨터에서 실행할 계획
		Socket socket = new Socket("localhost", 7777);
		
		// 이 부분부터는 서버와 연결이 완료된 상태이어야만 실행되는 코드이다.
