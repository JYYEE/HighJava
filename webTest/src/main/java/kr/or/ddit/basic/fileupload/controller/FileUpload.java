 ...
 * 1) location : 업로드한 파일이 임시적으로 저장될 경로를 지정한다. (기본값 : "" - 서버에서 임시파일이 저장되는 영역을 사용.) 
 ...
 * 속성을 작성할 때는 애노테이션 다음 괄호안에 작성. 직접 계산할 필요 없이 계산식으로 입력가능. 
 * jsp 파일에서 enctype="multipart/form-data"를 사용했을 때는 servlet파일에서 @MultipartConfig 를 반드시 사용해야 전송한 데이터 받을 수 있다.  
 */
...
public class FileUpload extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 파일이 아닌 일반데이터는 getParameter()메서드나 getParameterValues() 메서드를 이용해서 구한다.
		// getParameterValues() 메서드 : 같은 이름을 가진 속성을 여러개 받을 때 사용
		 ...
		 * 			'part이름' ==> <form>태그 안의 입력 요소의 name 속성값을 구별한다. 
		 */
		...
		for (Part part : request.getParts()) {
			...
			if(!"".equals(fileName)) { // 파일인지 검사
				...
				// 실제 저장되는 파일 이름이 중복되는 것을 방지하기 위해서 UUID객체를 이용하여 자장할 파일명을 만든다.
				String saveFileName = UUID .randomUUID().toString()+"_"+fileName; 
				// randomUUID() : 중복되지 않는 글자를 만들어냄. 파일명이 중복되면 저장된 파일 위로 덮어쓰기 되므로 이를 방지하기 위함.
				...
				try {
					...
					part.write(uploadPath + File.separator + saveFileName);	// 파일 저장하기... // File.separator : 필요에 따라 "/" 또는 "\"를 제공해줌 
				} catch (Exception e) {
					e.printStackTrace();
				}
		...
		IFileinfoService service = FileinfoServiceImpl.getInstance(); // service객체 불러오기
	 ...
	 * - Part 구조
	 * 1) 파일이 아닌 일반데이터일 경우
	 * ---------------1204ueasofxf0234u4te ==> Part를 구분하는 구분선(뒤에 난수값)
	 * content-disposition : form-data; name="username" ==> username : 파라미터명(form태그의 name 속성값)
	 * 					==> 빈줄
	 * gildong			==> 파라미터 값
	 * 
	 * 2) 파일일 경우
	 * ---------------1204ueasofxf0234u4te ==> Part를 구분하는 구분선
	 ...
	 * 
	 */
	// key값: value값. 각각의 영역을 세미콜론으로 분리
	//--------------------------------------------------------------------------
	
	
	// Part 구조 안에 파일명을 찾는 메서드
