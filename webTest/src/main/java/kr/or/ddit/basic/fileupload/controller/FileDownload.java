...
public class FileDownload extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(downFile.exists()) { // 파일이 있을 때만 작업해야 한다.
			...
			//response.setContentType("application/json; charset=utf-8"); // json으로 보낼 때
			response.setContentType("application/octet-stream; charset=utf-8"); // 파일 보낼 때
			
			// Response객체에 'content-disposition'헤더 속성을 추가한다. 
			String headerKey = "content-disposition";
			
			// 위 헤더 속성의 value값에는 클라이언트가 다운 받아서 저장할 파일명을 지정한다. 
			// ==> 업로드할 때의 원본 파일명을 지정해 준다.
			//String headerValue = "attachment; filename=\""+fvo.getOrigin_file_name() + "\""; // 한글 파일명 인코딩 수정 전
			String headerValue = "attachment; "+getEncodedFileName(request, fvo.getOrigin_file_name()); // 한글 파일명 인코딩 수정 후
