...
public class ServletTest03 extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String method = request.getMethod(); // 클라이언트의 전송 방식 구하기 (form태그에서 get, post를 method로 설정)
		System.out.println("method : " + method );
		if("GET".equals(method)) { // 대문자와 비교할 것!!
			this.doGet(request, response);
		//} else if("POST".equals(method)){ // 전송방법이 get, post이외에도 사용할 때
		} else {	
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<html><head><meta charset='utf-8'></head>"
				+ "<body><h1 style='color : red'>doGet메서드 처리 내용</h1></body></html>");
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		PrintWriter out = response.getWriter();
		out.println("<html><head><meta charset='utf-8'></head>"
				+ "<body><h1 style='color : blue'>doPost메서드 처리 내용</h1></body></html>");
	}
