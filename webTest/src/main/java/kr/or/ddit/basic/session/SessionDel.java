...
public class SessionDel extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 3. Session자체 삭제하기 ==> Session 객체의 invalidate() 메서드 이용하기
		// 형식) session객체.invalidate();
		// 		==> Session객체 자체가 삭제된다. 모든 데이터가 한꺼번에 삭제된다.
		//session.invalidate();
		
		
		out.println("<html><body><meta charset='utf-8'><title>Session 삭제 연습</title></head>");
		out.println("<body>");
