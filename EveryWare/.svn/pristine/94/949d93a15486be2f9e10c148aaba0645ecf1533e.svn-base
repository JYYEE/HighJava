		...
		success : function(res) {
			...
			$.each(res, function(i,v) {
				/*if(i < 3) {*/
					...
					code += `<tr><td>${v.sch_name}</td><td style="color: orange;">D-${dday}</td></tr>`;
				/*}*/
			})
			code += "</table>";
			$('#schedulediv').empty();
...
$.documentList = function(){
   $.ajax({
      ...
      success: function(res) {
         ...
         codea = "";
         if(res.length == 0){
			 			codea += '<tr><td colspan="3">결재할 문서가 없습니다.</td></tr>'
				 } else{
	         $.each(res, function(i,v) {
	            if(i < 3) {
	               codea += `<tr><td id="${mypath}/selectDoc.do?id=${v.doc_id}">${v.doc_title}</td><td>${v.emp_name}</td><td>${v.doc_date}</td></tr>`;
	            }
	         })
				 }
         
         $('#approvediv table').append(codea);
      },
      error : function(xhr) {
         alert(xhr.status);
      },
