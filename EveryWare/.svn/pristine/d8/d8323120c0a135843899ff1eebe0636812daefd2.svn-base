	...
	fetch(`${url}?${params}`)
		...
		.then(rs => {
			...
			function drawWeather(rs) {
				...
				document.getElementById('myclouds').innerHTML = rs.list[0].clouds.all + '&#37;';
				document.getElementById('mywind').innerHTML = wind + ' m/s';
				document.getElementById('myhumidity').innerHTML = rs.list[0].main.humidity + '&#37;';
