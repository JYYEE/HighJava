...
public class LprodMybatisTest {
	...
	public static void main(String[] args) {
		...
		// 스트림 객체 필요 문자기반, 바이트 기반 스트림 둘 다 상관 없음
		Reader rd = null;
...
//			// 논리값을 안준 상태이므로 기본값인 false로 인식 => 비활성화된 상태
//			// (쿼리문 필요하면 sqlsessionfactory 객체 만들고)
//
...
//		}
		...
		try {
			...
			// 이 메서드는 여러개의 레코드 각각을 VO객체에 담은 후 이 VO객체를 List에 추가해 주는 작업을 자동으로 수행한다.
			// 형식) session.selectList("namespace속성값.id속성값", 파라미터클래스)
			// 		==> 반환값 : resultType에 지정한 객체가 저장된 List객체
			// 제네릭타입은 resultType과 맞춰준다.
